...
def get_user_choice():
    position = ['Savdhan', 'Vishram', 'Baye Salute', 'Daine Salute', 'Front Salute' , 'Khade_Khade_Daine_Mud','Khade_Khade_Baye_Mud', 'Khade_Khade_Peeche_Mud']
...
def vishram(results, frame):
    ...
    knee_left.visibility < 0.7 or knee_right.visibility < 0.7:
        cv2.putText(frame, "Take a correct stance", (int(width/2), int(height/2)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        return False, frame

    shoulder_right_coordinates = (int(shoulder_right.x * width), int(shoulder_right.y * height))
    wrist_right_coordinates = (int(wrist_right.x * width), int(wrist_right.y * height))
    shoulder_left_coordinates = (int(shoulder_left.x * width), int(shoulder_left.y * height))
    wrist_left_coordinates = (int(wrist_left.x * width), int(wrist_left.y * height))
    hip_right_coordinates = (int(hip_right.x * width), int(hip_right.y * height))
    hip_left_coordinates = (int(hip_left.x * width), int(hip_left.y * height))
    ear_right_coordinates = (int(ear_right.x * width), int(ear_right.y * height))
    ...
    heel_left_coordinates =  (int(heel_left.x * width), int(heel_left.y * height))
    heel_right_coordinates = (int(heel_right.x * width), int(heel_right.y * height))

    #get the center of the body
    shoulder_center_coordinates = midpoint(shoulder_left_coordinates, shoulder_right_coordinates)
    hip_center_coordinates = midpoint(hip_left_coordinates, hip_right_coordinates)
    ear_center_coordinates = midpoint(ear_left_coordinates, ear_right_coordinates)
    knee_center_coordinates = midpoint(knee_left_coordinates, knee_right_coordinates)
    ...
    distance_between_heels = get_distance(heel_left_coordinates, heel_right_coordinates)

    back_posture_check = back_posture > 160 and back_posture < 200
    body_posture_check = body_posture > 160 and body_posture < 200
    distance_between_heels_check = distance_between_heels> 60 and distance_between_heels < 120
    wrist_visibilty_check = wrist_right.visibility < 0.3 and wrist_left.visibility < 0.3

    if  back_posture_check and body_posture_check and distance_between_heels_check and wrist_visibilty_check:
        cv2.putText(frame, "Correct Vishram Position", (window_size[0]-800,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        return True, frame

    else:
        cv2.putText(frame, "Incorrect Vishram Position", (window_size[0]-800,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        
        if not back_posture_check:
            cv2.putText(frame, "Incorrect Back Posture", (window_size[0]-800,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        if not body_posture_check:
            cv2.putText(frame, "Incorrect Body Posture", (window_size[0]-800,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        if not distance_between_heels_check:
            cv2.putText(frame, "Incorrect Distance between Heels", (window_size[0]-800,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        if not wrist_visibilty_check:
            cv2.putText(frame, "Wrists should not be visible", (window_size[0]-800,window_size[1]-20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        return False, frame

def savdhan_front(results, frame):
    ...
    shoulder_right_coordinates = (int(shoulder_right.x * width), int(shoulder_right.y * height))
    elbow_right_coordinates = (int(elbow_right.x * width), int(elbow_right.y * height))
    wrist_right_coordinates = (int(wrist_right.x * width), int(wrist_right.y * height))
    shoulder_left_coordinates = (int(shoulder_left.x * width), int(shoulder_left.y * height))
    elbow_left_coordinates = (int(elbow_left.x * width), int(elbow_left.y * height))
    wrist_left_coordinates = (int(wrist_left.x * width), int(wrist_left.y * height))
    hip_right_coordinates = (int(hip_right.x * width), int(hip_right.y * height))
    hip_left_coordinates = (int(hip_left.x * width), int(hip_left.y * height))
    ankle_right_coordinates = (int(ankle_right.x * width), int(ankle_right.y * height))
    ankle_left_coordinates = (int(ankle_left.x * width), int(ankle_left.y * height))
    ear_right_coordinates = (int(ear_right.x * width), int(ear_right.y * height))
    ear_left_coordinates = (int(ear_left.x * width), int(ear_left.y * height))
    knee_left_coordinates = (int(knee_left.x * width), int(knee_left.y * height))
    knee_right_coordinates = (int(knee_right.x * width), int(knee_right.y * height))
    ...
    shoulder_center_coordinates = midpoint(shoulder_left_coordinates, shoulder_right_coordinates)
    hip_center_coordinates = midpoint(hip_left_coordinates, hip_right_coordinates)
    ankle_center_coordinates = midpoint(ankle_left_coordinates, ankle_right_coordinates)
    ear_center_coordinates = midpoint(ear_left_coordinates, ear_right_coordinates)
    knee_center_coordinates = midpoint(knee_left_coordinates, knee_right_coordinates)
    ...
    distance_between_knees = get_distance(knee_left_coordinates, knee_right_coordinates)
    distance_between_ankles = get_distance(ankle_left_coordinates, ankle_right_coordinates)
    distance_between_rightwrist_righthip = get_distance(wrist_right_coordinates, hip_right_coordinates)
    distance_between_leftwrist_lefthip = get_distance(wrist_left_coordinates, hip_left_coordinates)
    ...
    if back_posture_check and body_posture_check and distance_between_knees_check and distance_between_ankles_check and distance_between_rightwrist_righthip_check and distance_between_leftwrist_lefthip_check:
        cv2.putText(frame, "Correct Front Savdhan Position", (window_size[0]-800,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        return True, frame
    else:
        cv2.putText(frame, "Incorrect Front Savdhan Position", (window_size[0]-800,window_size[1]-350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not back_posture_check:
            cv2.putText(frame, "Incorrect back posture", (window_size[0]-600,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not body_posture_check:
            cv2.putText(frame, "Incorrect body posture", (window_size[0]-600,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not distance_between_knees_check:
            cv2.putText(frame, "Put knees close together", (window_size[0]-600,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not distance_between_ankles_check:
            cv2.putText(frame, "Put ankles close together", (window_size[0]-600,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not distance_between_rightwrist_righthip_check:
            cv2.putText(frame, "Put right wrist close to right hip", (window_size[0]-600,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)    
        if not distance_between_leftwrist_lefthip_check:
            cv2.putText(frame, "Put left wrist close to left hip", (window_size[0]-600,window_size[1]-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        return False, frame
...
def savdhan_back(results, frame):
    ...
    if  (not results.face_landmarks): 
        ...
        distance_between_leftwrist_lefthip > 19 and distance_between_leftwrist_lefthip < 66:
            cv2.putText(frame, "Correct Back Savdhan Position", (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            return True, frame
        else:
            cv2.putText(frame, "Incorrect Back Savdhan Position", (window_size[0]-500,window_size[1]-350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, "Back Posture: " + str(back_posture), (window_size[0]-500,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, "Body Posture: " + str(body_posture), (window_size[0]-500,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, "Distance Between Knees: " + str(distance_between_knees), (window_size[0]-500,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, "Distance Between Ankles: " + str(distance_between_ankles), (window_size[0]-500,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, "Distance Between Right Wrist and Right Hip: " + str(distance_between_rightwrist_righthip), (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, "Distance Between Left Wrist and Left Hip: " + str(distance_between_leftwrist_lefthip), (window_size[0]-500,window_size[1]-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            return False, frame
    else:
        return False, frame
...
def right_side_savdhan(results, frame):
    ...
    knee_right.visibility < 0.7:
        cv2.putText(frame, "Face the right side", (int(width/2), int(height/2)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        return False, frame
    ...
    knee_left_coordinates = (int(knee_left.x * width), int(knee_left.y * height))
    knee_right_coordinates = (int(knee_right.x * width), int(knee_right.y * height))
    left_toe_coordinates = (int(left_toe_from_pose.x * width), int(left_toe_from_pose.y * height))
    right_toe_coordinates = (int(right_toe_from_pose.x * width), int(right_toe_from_pose.y * height))
    left_heel_coordinates = (int(left_heel_from_pose.x * width), int(left_heel_from_pose.y * height))
    right_heel_coordinates = (int(right_heel_from_pose.x * width), int(right_heel_from_pose.y * height))
    ear_center_coordinates = (int((ear_left_coordinates[0] + ear_right_coordinates[0])/2), int((ear_left_coordinates[1] + ear_right_coordinates[1])/2))
    shoulder_center_coordinates = (int((shoulder_left_coordinates[0] + shoulder_right_coordinates[0])/2), int((shoulder_left_coordinates[1] + shoulder_right_coordinates[1])/2))
    hip_center_coordinates = (int((hip_left_coordinates[0] + hip_right_coordinates[0])/2), int((hip_left_coordinates[1] + hip_right_coordinates[1])/2))
    knee_center_coordinates = (int((knee_left_coordinates[0] + knee_right_coordinates[0])/2), int((knee_left_coordinates[1] + knee_right_coordinates[1])/2))

    back_posture = get_angle_between_three_points(ear_center_coordinates, shoulder_center_coordinates, hip_center_coordinates)
    body_posture = get_angle_between_three_points(shoulder_center_coordinates, hip_center_coordinates, knee_center_coordinates)

    distance_between_rightshoulder_leftshoulder = get_distance(shoulder_right_coordinates, shoulder_left_coordinates)
    distance_between_rightknee_leftknee = get_distance(knee_right_coordinates, knee_left_coordinates)
    distance_between_rightwrist_righthip = get_distance(wrist_right_coordinates, hip_right_coordinates)
    distance_between_leftwrist_lefthip = get_distance(wrist_left_coordinates, hip_left_coordinates)

    distance_left = check_direction(left_toe_coordinates, left_heel_coordinates)
    distance_right = check_direction(right_toe_coordinates, right_heel_coordinates)
    ...
    if distance_left > 0 and distance_right > 0 and \
        ...
        distance_between_leftwrist_lefthip < 25:
            cv2.putText(frame, f'Correct Right Savdhan', (window_size[0]-500,window_size[1]-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            return True, frame

    else:
        cv2.putText(frame, f'Incorrect Right Savdhan', (window_size[0]-500,window_size[1]-350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Back Posture: {back_posture}', (window_size[0]-500,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Body Posture: {body_posture}', (window_size[0]-500,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        ...
        cv2.putText(frame, f'Distance between right knee and left knee: {distance_between_rightknee_leftknee}', (window_size[0]-500,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Distance between right wrist and right hip: {distance_between_rightwrist_righthip}', (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Distance between left wrist and left hip: {distance_between_leftwrist_lefthip}', (window_size[0]-500,window_size[1]-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        return False, frame
...
def left_side_savdhan(results, frame):
    ...
    knee_left.visibility < 0.7:
        cv2.putText(frame, "Face the left side", (int(width/2), int(height/2)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        return False, frame
    ...
    if distance_left < 0 and distance_right < 0 and \
        ...
        distance_between_leftwrist_lefthip < 45:
            cv2.putText(frame, f'Correct Left savdhan', (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            return True, frame
    else:
        cv2.putText(frame, f'Incorrect Left savdhan', (window_size[0]-500,window_size[1]-350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Back posture: {back_posture}', (window_size[0]-500,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Body posture: {body_posture}', (window_size[0]-500,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Distance between right shoulder and left shoulder: {distance_between_rightshoulder_leftshoulder}', (window_size[0]-500,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Distance between right knee and left knee: {distance_between_rightknee_leftknee}', (window_size[0]-500,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
def baye_salute(results, frame):
    ...
    left_hand_middle_finger_mid = results.left_hand_landmarks.landmark[11] if results.left_hand_landmarks else None

    print('_'*45)
    if left_eyebrow is None:
        print(f'Left eyebrow is not visible - {left_eyebrow}')
    if left_hand_middle_finger_tip is None:
        print(f'Left hand middle finger tip is not visible - {left_hand_middle_finger_tip}')
    if left_hand_middle_finger_mid is None:
        print(f'Left hand middle finger mid is not visible - {left_hand_middle_finger_mid}')

    left_eyebrow_coordinates = (int(left_eyebrow.x * width), int(left_eyebrow.y * height)) if left_eyebrow else None
    left_hand_middle_finger_tip_coordinates = (int(left_hand_middle_finger_tip.x * width), int(left_hand_middle_finger_tip.y * height)) if left_hand_middle_finger_tip else None
    left_hand_middle_finger_mid_coordinates = (int(left_hand_middle_finger_mid.x * width), int(left_hand_middle_finger_mid.y * height)) if left_hand_middle_finger_mid else None

    is_distance_between_left_tip_eyebrow_correct = get_distance(left_eyebrow_coordinates, left_hand_middle_finger_tip_coordinates) <= 1.75 * get_distance(left_hand_middle_finger_tip_coordinates, left_hand_middle_finger_mid_coordinates) if left_eyebrow and left_hand_middle_finger_tip and left_hand_middle_finger_mid else False
    ...
    distance_between_rightknee_leftknee_check and angle_between_wrist_elbow_shoulder_left_check and is_distance_between_left_tip_eyebrow_correct:
            cv2.putText(frame, f'Correct Baye Salute', (window_size[0]-800,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            return True, frame
    else:
        cv2.putText(frame, f'Incorrect Baye Salute', (window_size[0]-800,window_size[1]-400), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not is_distance_between_left_tip_eyebrow_correct:
            cv2.putText(frame, f'Incorrect right hand salute', (window_size[0]-800,window_size[1]-350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not body_posture_check:
            cv2.putText(frame, f'Incorrect Body Posture', (window_size[0]-800,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not back_posture_check:
            cv2.putText(frame, f'Incorrect Back Posture', (window_size[0]-800,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not distance_between_rightshoulder_leftshoulder_check:
            cv2.putText(frame, f'Please face left side', (window_size[0]-800,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not distance_between_rightknee_leftknee_check:
            cv2.putText(frame, f'Keep legs close together', (window_size[0]-800,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not angle_between_wrist_elbow_shoulder_left_check:
            cv2.putText(frame, f'Incorrect salute', (window_size[0]-800,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        return False, frame
...
def Daine_Salute(results, frame):
    ...
    distance_between_rightknee_leftknee_check and angle_between_wrist_elbow_shoulder_left_check and is_distance_between_left_tip_eyebrow_correct:
            cv2.putText(frame, f'Correct Daine Salute', (window_size[0]-800,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            return True, frame
    else:
        cv2.putText(frame, f'Incorrect Daine Salute', (window_size[0]-800,window_size[1]-400), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not is_distance_between_left_tip_eyebrow_correct:
            cv2.putText(frame, f'Incorrect right hand salute', (window_size[0]-800,window_size[1]-350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not back_posture_check:
            cv2.putText(frame, f'Incorrect back posture', (window_size[0]-800,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not body_posture_check:
            cv2.putText(frame, f'Incorrect body posture', (window_size[0]-800,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not distance_between_rightshoulder_leftshoulder_check:
            cv2.putText(frame, f'Please face right', (window_size[0]-800,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not distance_between_rightknee_leftknee_check:
            cv2.putText(frame, f'Knees should be close to each other', (window_size[0]-800,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if not angle_between_wrist_elbow_shoulder_left_check:
            cv2.putText(frame, f'do proper salute', (window_size[0]-800,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        return False, frame
...
def front_salute(results, frame):
    ...
    if back_posture_check and body_posture_check and distance_between_knees_check and \
        distance_between_ankles_check and distance_between_rightwrist_righthip_check and angle_between_wrist_elbow_shoulder_left_check and is_distance_between_left_tip_eyebrow_correct:
        cv2.putText(frame, "Correct Front Salute Position", (window_size[0]-600,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        return True, frame
    else:
        cv2.putText(frame, "Incorrect Front Salute Position", (window_size[0]-600,window_size[1]-400), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if is_distance_between_left_tip_eyebrow_correct == False:
            cv2.putText(frame, "Incorrect Right Hand Position", (window_size[0]-800,window_size[1]-350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if back_posture_check == False:
            cv2.putText(frame, "Incorrect Back Posture", (window_size[0]-800,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if body_posture_check == False:
            cv2.putText(frame, "Incorrect Body Posture", (window_size[0]-800,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if distance_between_knees_check == False:
            cv2.putText(frame, "Incorrect Distance Between Knees", (window_size[0]-800,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if distance_between_ankles_check == False:
            cv2.putText(frame, "Incorrect Distance Between Ankles", (window_size[0]-800,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if distance_between_rightwrist_righthip_check == False:
            cv2.putText(frame, "Incorrect Distance Between Right Wrist and Right Hip", (window_size[0]-800,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        if angle_between_wrist_elbow_shoulder_left_check == False:
            cv2.putText(frame, "Incorrect Angle Between Wrist, Elbow and Shoulder Left", (window_size[0]-800,window_size[1]-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
def Daine_transition_pos1(results, frame):
    ...
    angle_between_knees_and_hip = get_angle_between_three_points(knee_left_coordinates, hip_center_coordinates, knee_right_coordinates)
    ...
    if distance_left > 0 and distance_right > 0 and \
        ...
        angle_between_knees_and_hip < 35:
            cv2.putText(frame, f'Correct Right transition', (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            return True, frame
    else:
        cv2.putText(frame, f'Incorrect Right Transition', (window_size[0]-500,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        ...
        cv2.putText(frame, f'Distance between shoulders: {distance_between_rightshoulder_leftshoulder}', (window_size[0]-500,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
def Baye_transition_pos1(results, frame):
    ...
    if distance_left < 0 and distance_right < 0 and \
        ...
        distance_between_leftwrist_lefthip < 45 and \
        angle_between_knees_and_hip < 35:
            cv2.putText(frame, f'Correct Left transition', (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            return True, frame
    else:
        cv2.putText(frame, f'Incorrect Left Transition', (window_size[0]-500,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Back Posture: {back_posture}', (window_size[0]-500,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Distance between shoulders: {distance_between_rightshoulder_leftshoulder}', (window_size[0]-500,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Distance between right wrist and right hip: {distance_between_rightwrist_righthip}', (window_size[0]-500,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f'Distance between left wrist and left hip: {distance_between_leftwrist_lefthip}', (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
def Piche_position(results, frame):
    ...
    if results.face_landmarks is None:
        ...
        #distance_between_rightknee_leftknee > 0 and distance_between_rightknee_leftknee < 10000
            cv2.putText(frame, f'Correct Piche transition', (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            return True, frame
        else:
            cv2.putText(frame, f'Incorrect Piche Transition', (window_size[0]-500,window_size[1]-300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            # cv2.putText(frame, f'Back posture: {back_posture}', (window_size[0]-500,window_size[1]-250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            # cv2.putText(frame, f'Distance between right shoulder and left shoulder: {distance_between_rightshoulder_leftshoulder}', (window_size[0]-500,window_size[1]-200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            # cv2.putText(frame, f'Distance between right wrist and right hip: {distance_between_rightwrist_righthip}', (window_size[0]-500,window_size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            # cv2.putText(frame, f'Distance between left wrist and left hip: {distance_between_leftwrist_lefthip}', (window_size[0]-500,window_size[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
def Khade_Khade_Daine_Mud(results, frame, mud_array):
    var1, frame = savdhan_front(results, frame)
    if var1:
    ...
    if flag:
        cv2.putText(frame, f'Khade Khade Daine Mud', (window_size[0]-550,window_size[1]-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
...
def Khade_Khade_Baye_Mud(results, frame, mud_array):
    ...
    flag, mud_array = Mud_fnc(var1,var2,var3,mud_array)
    if flag:
        cv2.putText(frame, f'Khade Khade Baye Mud', (window_size[0]-550,window_size[1]-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    return flag, frame, mud_array
...
def Khade_Khade_Peeche_Mud(results, frame, mud_array):
    ...
    flag, mud_array = Mud_fnc(var1,var2,var3,mud_array)
    if flag:
        cv2.putText(frame, f'Khade Khade Peeche Mud complete', (window_size[0]-550,window_size[1]-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    return flag, frame, mud_array